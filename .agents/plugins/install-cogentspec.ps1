...
$requiredSparsePaths = @('.agents/plugins', 'plugins/cogentspec', 'plugins/cogentstack')
...
try {
    ...
    $installJson = Invoke-BoundedNative -FilePath $script:codexPath -Arguments @('plugin', 'add', 'cogentspec@cogentstack', '--json') -Operation 'plugin installation'
    ...
    $installedPlugin = @($pluginList.installed | Where-Object { $_.pluginId -eq 'cogentspec@cogentstack' }) | Select-Object -First 1
    ...
    $sourcePluginPath = Join-Path $marketplaceRoot 'plugins\cogentspec'
    ...
    $allowedFiles = @(
        ...
        'skills/cogentspec/scripts/delete-specification-project.ps1',
    ...
    $requiredSkillStatements = @(
        ...
        'run `scripts/project-build-handoff.ps1 -ContextKey <context>` exactly once instead',
        'Never ask the user to run `$cogentspec` or repeat **Build my project** merely to refresh task tools.',
        ...
        'call it exactly once with `target: { type: "browser", url: chatgptWorkspaceUrl }`',
        'Treat either `opened` or `queued` as acceptance and do not retry.',
    ...
    $bridgeScript = Get-Content -LiteralPath (Join-Path $installedPath 'skills\cogentspec\scripts\start-cogentstack-bridge.ps1') -Raw
    ...
    if ($UpdateOnly) {
        ...
        $resetScript = Join-Path $installedPath 'skills\cogentspec\scripts\reset-cogentspec-update.ps1'
