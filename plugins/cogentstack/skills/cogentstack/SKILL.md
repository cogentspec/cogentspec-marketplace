...
5. After a verified update, stop before resolving the project or starting the Bridge. Report the installed version and tell the user to start a new task before invoking `$cogentstack`, because this task still has the earlier skill instructions loaded.
...
3. Return both exact work-area links and let the user choose where to open the shared task workspace. Label `webWorkspaceUrl` **Open this task’s CogentSpec workspace in web** and label `chatgptWorkspaceUrl` **Open this task’s CogentSpec workspace in ChatGPT**. Each short-lived fragment establishes a separate browser session for the same context and must be returned only in its link, never described or logged separately. Opening one must not consume the other. Keep `workspaceUrl` only as the compatibility alias for `chatgptWorkspaceUrl`, and keep `https://cogentspec.com/stack` as the Bridge connection surface. Do not open either workspace link, call a browser-control tool, create or select a browser tab, inspect unrelated tabs, hide a sidebar, resize a window, arrange a split, or mount an embedded panel.
4. Report only the verified connection state. Readiness without `status: ready` is not a successful connection.

The user decides which returned CogentSpec.app link to open and may open both. Both views use the same server-authoritative context, active project, approved decisions, Bridge queue, Git state, preview state, and deployment state. Saved changes refresh from the shared server without copying unsaved browser-local text between views. Switching AI projects changes the context in both URLs, creates two fresh one-use workspace handoffs, and starts or reuses that context's Bridge worker; it does not create permanent CogentSpec browser tabs.
