...
3. Call the CogentSpec MCP tool `get_project_build_handoff` exactly once with that exact context key. Require `status: ready`, `command: Build my project`, the same project request identifier, Project Type, Contract Runtime, and Contract Pack as the local manifest. If the handoff is unavailable or incomplete, say only that CogentSpec could not start the build and direct the user to run `$cogentstack` once before trying again. Do not mention MCP, OAuth, tool names, plugin paths, ask the user to repeat the command, or reconstruct the brief.
