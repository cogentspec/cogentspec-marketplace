...
3. Return both exact work-area links and let the user choose where to open the shared task workspace. Render each choice as an HTML anchor with `target="_blank"` and `rel="noopener noreferrer"` so a normal left-click opens a separate tab instead of trying to replace the current AI task view. Label `webWorkspaceUrl` **Open this task’s CogentSpec workspace in web** and label `chatgptWorkspaceUrl` **Open this task’s CogentSpec workspace in ChatGPT**. Each short-lived fragment establishes a separate browser session for the same context and must be returned only in its anchor's `href`, never described or logged separately. Opening one must not consume the other. Keep `workspaceUrl` only as the compatibility alias for `chatgptWorkspaceUrl`, and keep `https://cogentspec.com/stack` as the Bridge connection surface. Do not open either workspace link, call a browser-control tool, create or select a browser tab, inspect unrelated tabs, hide a sidebar, resize a window, arrange a split, or mount an embedded panel.
