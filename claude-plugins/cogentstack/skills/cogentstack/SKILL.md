...
3. Return the exact private `workspaceUrl` on `https://cogentspec.app` as a clickable work-area link. The short-lived fragment establishes a separate workspace session and must be returned only in the link, never described or logged separately. Keep `https://cogentspec.com/stack` as the Bridge connection surface. Do not open the workspace link, call browser control, create or select a browser tab, inspect browser profiles, hide the Claude sidebar, resize either window, or create an embedded companion panel. The user decides when and where to open the CogentSpec.app workspace.
